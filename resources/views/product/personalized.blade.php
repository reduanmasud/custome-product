...
@section('head_content')
    ...
    <style>
        .canvas-container {
            ...
            background-image: url("{{ URL::asset('product_upload') }}/{{ $product->variations->find($variation)->image_url }}") !important;
...
@section('content')
    ...
    <script>
        function buyitem() {
            ...
            window.location.href = "{{ route('product.buy', ['id' => $product->id, 'var_id' => $variation]) }}"
        ...
        try {
            ...
            var imgEditor = new ImageEditor('#image-editor-container', buttons, [],
                "{{ URL::asset('product_upload') }}/{{ $product->mockup }}");
            ...
            browserWarning.innerHTML =
                '<p style="line-height: 26px; margin-top: 100px; font-size: 16px; color: #555">Yo{{ ur browser is out of date!<br/>Please update to a modern browser, for example:<a href="https://www.google.com/chrome/" target="_blank">Chrome</a>!</p>';
